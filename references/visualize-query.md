...
For a standalone NDB-only database, no nostos.toml is needed. Explicitly authorize the reviewed binary and database:

    python3 <skill-root>/scripts/nostos_core.py resolve \
      --binary /absolute/path/to/nostos --database /absolute/path/to/graph.ndb --json
    python3 <skill-root>/scripts/nostos_core.py run \
      --binary /absolute/path/to/nostos --database /absolute/path/to/graph.ndb -- \
      query --read-only \
