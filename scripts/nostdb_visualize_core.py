#!/usr/bin/env python3
"""Run a strictly read-only NostDB CLI surface for graph visualization."""
    ...
    while index < len(arguments):
        ...
        if argument == "--read-only":
            ...
            raise VisualizationError(
                "query option {} is not allowed by nostdb-visualize".format(argument)
            )
        elif query is None:
            query = argument
        else:
...
def safe_inspection(command: str, arguments: List[str], database: Path) -> List[str]:
    ...
    while index < len(arguments):
        ...
        if argument != "--format":
            raise VisualizationError(
                "{} option {} is not allowed by nostdb-visualize".format(
                    command, argument
                )
            )
        output_format = option_value(arguments, index, argument)
...
def main() -> int:
    ...
    try:
        ...
        print("nostdb-visualize: {}".format(error), file=sys.stderr)
        return 3
