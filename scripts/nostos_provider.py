...
def native_candidate(project: Path, explicit: Optional[str]) -> Optional[Path]:
    ...
    selected = explicit or os.environ.get("NOSTOS_BIN")
    if selected:
...
def resolve_provider(
    ...
    if values.get("core_binary") and not explicit and not os.environ.get("NOSTOS_BIN"):
        print(
            "nostos-core: ignoring untrusted skills.core_binary metadata; "
            "authorize a reviewed binary with --binary PATH or NOSTOS_BIN=PATH",
            file=sys.stderr,
        )
    if policy == "npx":
        if explicit:
    ...
    if policy == "auto":
        return npx_provider(expected)
    raise CoreResolutionError(
        "cannot locate nostos {}; pass a reviewed binary with --binary, set "
        "NOSTOS_BIN, or put nostos on PATH; skills.core_binary is metadata only "
