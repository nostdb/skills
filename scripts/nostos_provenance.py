...
def main() -> int:
    ...
    try:
        ...
        print("nostos-provenance: cannot read {}: {}".format(source, error), file=sys.stderr)
        return 7
    if args.source_label:
        ...
        try:
            ...
            print(
                "nostos-provenance: external sources require --source-label to avoid absolute paths",
                file=sys.stderr,
            )
            return 2
    if not label or label.startswith("/") or ".." in Path(label).parts or "\n" in label or "\r" in label:
        print("nostos-provenance: source label must be a normalized relative path", file=sys.stderr)
        return 2
    source_hash = hashlib.sha256(payload).hexdigest()
    if args.expected_sha256 and args.expected_sha256 != source_hash:
        print(
            "nostos-provenance: source conflict: expected {}, found {}".format(
