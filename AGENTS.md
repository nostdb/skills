...
- Every public Skill must bundle all runtime references and scripts below its own directory. Do not use a relative path that escapes the Skill, depend on the repository checkout or current working directory, assume an agent-specific parent, or require another NostosDB Skill.
- NostosDB-maintained Codex/Claude adapters are not part of the public installation contract.
