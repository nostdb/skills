...
- Skills may eventually write `.nost`, but they must never write `.nostdb` directly.
- Do not implement a parser, storage layer, or binary writer.
- Follow the active Root Stage. Keep deterministic helpers dependency-free and route parsing, formatting, validation, synchronization, queries, and `.nostdb` writes through the public CLI.
