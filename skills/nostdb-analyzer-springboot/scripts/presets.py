...
The `nostdb` Skill's scripts are shell, and this one is not. Shell is right when the work *is* running
commands, which is what that Skill does. This one reads a table and matches a name, and shell has no data
structures for either: the index is pipe separated precisely because a shell reader could not be trusted with
JSON, and every field access went through `awk`. Python reads the same file in a `split` and matches on a set.
...
class Preset:
    ...
    def __init__(self, name: str, document: str, covers: str, describes: str) -> None:
        ...
        self.covers = {held.strip() for held in covers.split(",") if held.strip()}
...
def command_for(presets: list[Preset], annotation: str) -> None:
    ...
    of them mean nothing to any preset here — dependency injection is deliberately uncovered, so `@Service`
    finding nothing is correct.
    """
    for preset in presets:
...
def main(argv: list[str]) -> None:
    ...
    if action == "list":
        ...
        fail("usage: presets.py [list | document NAME | for ANNOTATION]", 2)
