...
- Never open, create, patch, copy as a mutation, or decode `.ndb`. Invoke only the pinned public `nostdb` CLI through `<skill-root>/scripts/nostdb_core.py`.
...
For standalone NDB-only use, provide both `--binary /reviewed/nostdb` and `--database /existing/graph.ndb`; no `nostdb.toml` is required. Every run requires an explicit existing database. The wrapper never forwards a project path to the CLI, so it cannot trigger implicit synchronization.

If inspection reports `source_managed: true`, `.nostdb` is authoritative. The visualization workflow still reads only the existing materialized database. Otherwise `.ndb` is authoritative. Visualization remains read-only in either mode.
