#!/usr/bin/env python3
"""Small dependency-free helpers for the versioned Nostos skill configuration."""
...
def config_path(project: Path) -> Path:
    return project.resolve() / "nostos.toml"
...
def require_core_version(project: Path) -> str:
    ...
    if version is None:
        raise ConfigError("nostos.toml is missing skills.core_version")
...
def atomic_write(path: Path, text: str) -> None:
    path = path.resolve()
    if path.name != "nostos.toml":
        raise ConfigError("configuration helper may write only nostos.toml")
    descriptor, temporary = tempfile.mkstemp(prefix=".nostos-config-", dir=str(path.parent))
