...
For standalone NDB-only use, provide both `--binary /reviewed/nostos` and `--database /existing/graph.ndb`; no `nostos.toml` is required. Every run requires an explicit existing database. The wrapper never forwards a project path to the CLI, so it cannot trigger implicit synchronization.

If inspection reports `source_managed: true`, `.nostos` is authoritative. The visualization workflow still reads only the existing materialized database. Otherwise `.ndb` is authoritative. Visualization remains read-only in either mode.
