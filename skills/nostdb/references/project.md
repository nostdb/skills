...
- `centralized`: use for a graph maintained together under `.nostdb/`.
...
Initialization refuses a nonempty directory. After inspecting an existing code/document project and confirming it is the intended destination, add `--allow-nonempty`; existing `nostdb.toml` and the selected source target are still never replaced.

The helper creates one canonical source entry, a nonzero Stable Module ID, and these independent pins in `nostdb.toml`:
...
[source]
layout = "centralized"
entry = ".nostdb/graph.nostdb"

[modules]
".nostdb/graph.nostdb" = "<stable-module-id>"
