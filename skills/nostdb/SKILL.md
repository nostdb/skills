---
name: nostdb
description: Initialize, explain, model, populate, inspect, query, validate, and maintain NostDB projects. Use for `nostdb init`, `nostdb help`, project setup, source layout, Schema or Constraint design, document or code ingestion, graph exploration, canonical formatting, synchronization, diagnostics, or any end-to-end NostDB workflow other than graph visualization.
---
...
1. Require the intended project directory. Inspect its files, `nostdb.toml`, `.nostdb` modules, imports, and Git changes; preserve unrelated work.
...
4. Use the selected task reference. Preserve Stable Module IDs and source ownership, operate on complete `.nostdb` files through the guarded source workflow, and treat Core diagnostics as authoritative.
...
Use `nostdb-visualize` when the requested outcome is a diagram or visualization dataset. Never open, generate, patch, or interpret `.ndb` bytes; only the resolved CLI may create or update the database.
